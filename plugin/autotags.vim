...
command! AutotagsUpdateCurrent call AutotagsUpdateCurrent()
command! AutotagsUpdateSubDir call AutotagsUpdateSubDir()

" ----------------------------------------------------------------------------------
fun! AutotagsRemoveAll()
    for l:entry in split(system("ls " . s:autotags_subdir), "\n")
        if stridx(l:entry, "include_") == 0
            let l:path = s:autotags_subdir . "/" . l:entry

            if getftype(l:path) == 'link' && isdirectory(l:path)
                let l:path = resolve(l:path . "/origin")
                let l:hash_path = g:autotagsdir . '/' . s:PathHash(l:path)
                if getftype(l:hash_path) == "dir"
                    echomsg "delete sub dir tag : " . l:hash_path  . " for " . l:path
                    call system("rm -r " . shellescape(l:hash_path))
                endif
            endif
        endif
    endfor
...
fun! AutotagsRemoveSubDir()
    let l:dir_del= s:AutotagsAskPath(getcwd(), "input tag dir to delete : ")
    if l:dir_del == ""
        return
    endif

    let l:hash_dir_del = g:autotagsdir . '/' . s:PathHash(l:dir_del)
    if getftype(l:hash_dir_del) == "dir"
        echomsg "delete sub dir tag : " . l:hash_dir_del
        call system("rm -r " . shellescape(l:hash_dir_del))
        call system("rm -r " . shellescape(s:autotags_subdir . "/include_" . s:PathHash(l:dir_del)))
    endif
...
" ----------------------------------------------------------------------------------
fun! AutotagsShowAll()
    echomsg "root dir : " . resolve(s:autotags_subdir . "/origin")
    for l:entry in split(system("ls " . s:autotags_subdir), "\n")
        if stridx(l:entry, "include_") == 0
            let l:path = s:autotags_subdir . "/" . l:entry
            while getftype(l:path) == 'link' && isdirectory(l:path)
                if getftype(l:path) == 'link' && isdirectory(l:path)
                    let l:path = resolve(l:path . "/origin")
                    echomsg "sub  dir : " . l:path
                endif
            endwhile
        endif
    endfor
...
" ----------------------------------------------------------------------------------
fun! AutotagsUpdateCurrent()
    echomsg "funtion todo"
endfun

" ----------------------------------------------------------------------------------
fun! AutotagsUpdateSubDir()
    echomsg "funtion todo"
endfun
" zsept add end
...
fun! AutotagsUpdate()
    if s:AutotagsIsLoaded() == 0
        ...
        let l:sourcedir = s:AutotagsValidatePath(l:sourcedir)
        ...
        for l:entry in split(system("ls " . s:autotags_subdir), "\n")
            if stridx(l:entry, "include_") == 0
                ...
                endwhile
                "call AutotagsAddPath(l:path)
                call AutotagsUpdatePath(l:path)
...
fun! AutotagsAddPath(sourcedir)
    ...
    call system("ln -s " . shellescape(l:tagsdir) . " " . shellescape(s:autotags_subdir . "/include_" . s:PathHash(l:sourcedir)))
